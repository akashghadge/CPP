...
same tech as queen kill combination expect isSafe function
here we check any prior knight killing current one or not
but here i check all ;)
*/
...
bool isSafe(vector<vector<int>> &chess, int row, int col)
{
    ...
    nr = row - 2;
    nc = col + 1;
    if (nr > 0 && nr < chess.size() && nc < chess.size() && nc > 0 && chess[nr][nc] != 0)
        return false;
