...
{
public:
    // here we use concept sliding window and hashing
    string findSubString(string str)
    {
        // Your code goes here
        // firstly we store all distinct element
        unordered_map<char, int> freq;
        for (char val : str)
        {
            freq[val]++;
        }
        // max distinct eleement
        ...
        {
            freq[str[i]]++;
            // if distinct ele occure we cnt++
            if (freq[str[i]] == 1)
            {
                currCount++;
            }
            // if we get all elements then just shrink window
            if (currCount == disEle)
            {
                // while shrinking window we check first charecter is duplicate or not
