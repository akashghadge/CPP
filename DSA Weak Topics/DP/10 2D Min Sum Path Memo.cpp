...
int solve(vvi &arr, int r, int c, int n, int m, vvi &dp)
{
    if (r >= n or c >= m)
        return INT_MAX;
    if (r == n - 1 and c == m - 1)
    ...
    if (dp[r][c] != -1)
        return dp[r][c];
    ll down = solve(arr, r + 1, c, n, m, dp);
    ll right = solve(arr, r, c + 1, n, m, dp);
    return dp[r][c] = min(down, right) + arr[r][c];
}
void sol()
{
    var2(n, m);
