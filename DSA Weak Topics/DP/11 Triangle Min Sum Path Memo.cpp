...
int temp(0);
int solve(vvi &arr, int n, int r, int c, vvi &dp)
{
    if (r >= n or c >= arr[r].size())
        return INT_MAX;
    if (r == n - 1)
        return arr[r][c];
    if (dp[r][c] != -1)
        return dp[r][c];
    temp++;
    int d = solve(arr, n, r + 1, c, dp);
    int cross = solve(arr, n, r + 1, c + 1, dp);
    return dp[r][c] = min(d, cross) + arr[r][c];
}
void sol()
{
    var(n);
