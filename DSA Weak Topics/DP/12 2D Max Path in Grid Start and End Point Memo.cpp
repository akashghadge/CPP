...
int temp = 0;
int solve(vvi &arr, int n, int m, int r, int c, vvi &dp)
{
    ...
    if (dp[r][c] != -1)
        return dp[r][c];
    temp++;
    int up = solve(arr, n, m, r - 1, c, dp);
    int left = solve(arr, n, m, r - 1, c - 1, dp);
    int right = solve(arr, n, m, r - 1, c + 1, dp);
...
void sol()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ans = max(ans, solve(arr, n, m, n - 1, i, dp));
    }
    prn(ans);
    prn(temp);
}
