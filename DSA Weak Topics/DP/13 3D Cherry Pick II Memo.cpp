...
int solve(vvi arr, int n, int m, int r, int c1, int c2, vector<vvi> &dp)
{
    if (r >= n or c1 < 0 or c2 < 0 or c1 >= m or c2 >= m)
        return INT_MIN;
    if (r == n - 1)
    {
        if (c1 == c2)
    ...
    for (int i = -1; i <= 1; i++)
    {
        for (int j = -1; j <= 1; j++)
        {
            ...
            curr = curr + solve(arr, n, m, r + 1, c1 + i, c2 + j, dp);
            mx = max(curr, mx);
        }
    }
