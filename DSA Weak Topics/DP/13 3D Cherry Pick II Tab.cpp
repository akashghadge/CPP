...
{
    vector<vvi> dp(n, vvi(m, vi(m, INT_MIN)));
    int base = n - 1;
    for (int j1 = 0; j1 < m; j1++)
    {
        ...
        {
            if (j1 == j2)
                dp[base][j1][j2] = arr[base][j1];
            else
                dp[base][j1][j2] = arr[base][j1] + arr[base][j2];
        }
    }
    for (int i = n - 2; i >= 0; i--)
    {
            ...
            for (int j2 = 0; j2 < m; j2++)
            {
                ...
                for (int d1 = -1; d1 <= 1; d1++)
                {
                    for (int d2 = -1; d2 <= 1; d2++)
                    {
                        int nj1 = j1 + d1;
                        int nj2 = j2 + d2;
                        if (nj1 < 0 or nj2 < 0 or nj1 >= m or nj2 >= m)
                            continue;
                        mx = max(mx, dp[i + 1][nj1][nj2]);
                    }
                }
                dp[i][j1][j2] = curr + mx;
            }
