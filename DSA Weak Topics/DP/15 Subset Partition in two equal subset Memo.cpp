...
bool solve(int id, int k, vi &arr, vvi &dp)
{
    if (k < 0)
        return false;
    if (k == 0)
    ...
    if (dp[id][k] != -1)
        return dp[id][k];
    bool pick = solve(id - 1, k - arr[id], arr, dp);
    bool not_pick = solve(id - 1, k, arr, dp);
    return dp[id][k] = pick or not_pick;
}
bool sol()
{
    var(n);
    ...
    bool ans = solve(n - 1, sum / 2, arr, dp);
    return ans;
}
