...
int solve(int id, int k, vi &arr, vvi &dp)
{
    ...
    int pick = INT_MAX;
    int curr_used = 1;
    while (arr[id] * curr_used <= k)
    {
        int curr_ans = solve(id - 1, k - (arr[id] * curr_used), arr, dp);
        if (curr_ans != INT_MAX)
            pick = min(pick, curr_ans + curr_used);
        curr_used++;
    }
    return dp[id][k] = min(pick, not_pick);
}
void sol()
{
    var2(n, k);
