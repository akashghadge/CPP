...
int solve(int id, int wt, vi &value, vi &weights, vvi &dp)
{
    if (wt == 0)
        return 0;
    if (id == 0)
    {
        if (wt >= weights[0])
            return value[0] * (wt / weights[0]);
        else
            return 0;
    }
    if (dp[id][wt] != -1)
        return dp[id][wt];
    int not_pick = solve(id - 1, wt, value, weights, dp);
    int pick = 0;
    if (weights[id] <= wt)
        pick = solve(id, wt - weights[id], value, weights, dp) + value[id];
...
void sol()
{
    ...
    varv(value, n);
    vvi dp(n, vi(wt + 1, -1));
    int ans = solve(n - 1, wt, value, weights, dp);
    prn(ans);
}
