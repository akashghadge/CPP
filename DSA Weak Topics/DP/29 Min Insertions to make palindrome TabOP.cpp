...
/*
This question not taken by pep coding
Intuition
    if very simple way is append entire string reversed form
    here if we have some string and make it to palindrome in min operations
    then just delete characters and so how many and which chars should
    delete
    we can delete the chars which does not appear in longest palindrome subseq
therefore,
ans = str.size() - lps(str)
*/
...
void sol()
{
    vars(str);
    int n = str.size();
    int ans = str.size() - solve(n, n, str, rev((str)));
    prn(ans);
}
