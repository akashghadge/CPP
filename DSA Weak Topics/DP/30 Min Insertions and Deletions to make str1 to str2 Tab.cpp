...
/*
here we have two string str1 and str2
intuition
    consider making str1 -> str2 we maximum needs n+m operations
    i.e delete every member of str1 and add str2
    but if we have some seq in common then by preserving that particular seq
    we can minimize operations
i.e
n+m-2*lcs(str1,str2);
*/
...
void sol()
{
    ...
    int n = str1.size();
    int m = str2.size();
    int ans = n + m - 2 * solve(n, m, str1, str2);
    prn(ans);
}
