...
/*
we have two stirng a and b
find how many subseq in a which matches b string
bbag
bag
*/
int solve(int n, int m, string a, string b, vvi &dp)
{
    if (n < 0)
        return 0;
    if (m < 0)
    ...
    int pick = 0;
    if (a[n] == b[m])
        pick = solve(n - 1, m - 1, a, b, dp);
    int not_pick = solve(n - 1, m, a, b, dp);
    return dp[n][m] = pick + not_pick;
}
void sol()
{
    vars(str1);
