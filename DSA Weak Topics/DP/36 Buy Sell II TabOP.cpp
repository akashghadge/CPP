...
/*
In this example
we have stock in array and can do infinite transaction
but only restriction is for selling stock there must be buy
and for buying there should not any stock is bought already

here we just add one parameter can_buy => buy
so we can track is we already bought stock or not
memo is top down so we go from 0 -> n i.e n is base case
and we will do bottom-up for tabulation cause opposite nature
and loop is also changed n-1 -> 0


pep coding approach is totally different cause
pep says sell on rises each rise is sellable
*/
ll solve(int n, vi &Arr)
{
    vector<ll> ahead(2, 0);
    vector<ll> cur(2, 0);
