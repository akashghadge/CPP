...
/*
In this example we can do at most two transactions
*/
int temp = 0;
ll solve(int id, int buy, int k, vi &arr, int n, vector<vvi> &dp)
{
    // ---base---
    if (id == n) // out of bound
        return 0;
    if (k == 0 and buy) // if transactions exhaust
        return 0;
    if (dp[id][buy][k] != -1)
        return dp[id][buy][k];
    ll profit = 0;
    ...
    if (buy) // can buy now
    {
        profit = max(profit, -arr[id] + solve(id + 1, !buy, k - 1, arr, n, dp));
        profit = max(profit, solve(id + 1, buy, k, arr, n, dp));
    }
    else // already bought
    {
        profit = max(profit, +arr[id] + solve(id + 1, !buy, k, arr, n, dp));
        profit = max(profit, solve(id + 1, buy, k, arr, n, dp));
    }
    return dp[id][buy][k] = profit;
}
void sol()
{
    var(n);
