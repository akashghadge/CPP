...
*/
ll solve(vi arr, int id, int buy, int n, int fee, vvi &dp)
{
    if (id >= n)
        return 0; // base case
    ...
    { // We can buy the stock
        profit = max(0 + solve(arr, id + 1, 0, n, fee, dp), -arr[id] + solve(arr, id + 1, 1, n, fee, dp));
    }

    if (buy == 1)
    { // We can sell the stock
        profit = max(0 + solve(arr, id + 1, 1, n, fee, dp), arr[id] - fee + solve(arr, id + 1, 0, n, fee, dp));
    }
