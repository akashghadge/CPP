/*
5 Maximum Sum non two adjacent
*/
...
int solve(int ind, int n, vi &arr, vi &dp)
{
    if (ind == 0)
        return arr[ind];
    if (ind < 0)
    ...
    if (dp[ind] != -1)
        return dp[ind];
    int pick = arr[ind] + solve(ind - 2, n, arr, dp);
    int not_pick = solve(ind - 1, n, arr, dp);
...
void sol()
{
    ...
    vi arr1 = arr;
    arr1.pop_back();
    vi arr2(arr.begin() + 1, arr.end());
    int ans1 = solve(m - 1, m, arr1, dp);
    int ans2 = solve(m - 1, m, arr2, dp2);
    prn(max(ans1, ans2));
}
