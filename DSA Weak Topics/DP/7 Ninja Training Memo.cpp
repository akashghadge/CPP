...
int temp = 0;
int solve(int id, int select, int m, vvi &arr, vvi &dp)
{
    int curr = arr[id][select];
    if (id == 0)
        return curr;
    if (dp[id][select] != -1)
        return dp[id][select];
    ...
    for (int i = 0; i < m; i++)
    {
        if (i == select)
            continue;
        rec = max(rec, solve(id - 1, i, m, arr, dp));
    }
    return dp[id][select] = (curr + rec);
}
void sol()
{
    var(n);
    ...
    vvi dp(n, vi(m, -1));
    int ans = solve(n - 1, 0, m, arr, dp);
    int ans1 = solve(n - 1, 1, m, arr, dp);
    int ans2 = solve(n - 1, 2, m, arr, dp);
    prn(max(max(ans, ans1), ans2));
    prn(temp);
}
