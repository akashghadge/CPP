...
ll solve(int r, int c, int n, int m, vvi &arr, vvi &dp)
{
    if (r >= n or c >= m or arr[r][c] == -1)
        return 0;
    if (r == n - 1 and c == m - 1)
        return 1;
    if (dp[r][c] != -1)
        return dp[r][c];
    ll down = solve(r + 1, c, n, m, arr, dp);
    ll right = solve(r, c + 1, n, m, arr, dp);
